...
void update_brush_particles(bool brushMode, bool& playerSunMode, float& playerSunTimer) {
    ...
    while (i < count) {
        ...
        if (type == BRUSH_DROP) {
            if (!brushParticles[i].absorbed) {
                ...
                if (hitWater) {
                    ...
                    if (rand() % 5 == 0) spawnRainbowFragments(brushParticles[i].x, brushParticles[i].y, 0, 0.2f);
                }
            }
        }
        ...
        brushParticles[i].t += (brushMode ? 0.12f : 0.06f);
        ...
        if (brushParticles[i].dissolveFrame > 0) {
            ...
            if (type == BRUSH_RAINBOW && brushParticles[i].dissolveFrame == 1) {
                spawnRainbowFragments(brushParticles[i].x, brushParticles[i].y, brushParticles[i].t, 0.01f); request_play(rainbowSound);
            }
                        ...
                        if (!p.isPlayer) {
                            ...
                            if (bp.type == BRUSH_BLUE) {
                                ...
                                if (dx * dx + dy * dy < (bp.baseSize * 0.7f) * (bp.baseSize * 0.7f)) {
                                    ...
                                    bp.dissolveFrame = 1;
                                    playerRainbow = true;
                                    playerRainbowTimer = 1000000.0f;
                                    playerJumpTimer = 0.5f;

                                    spawnRainbowFragments(bp.x, bp.y, bp.t, 0.02f);
