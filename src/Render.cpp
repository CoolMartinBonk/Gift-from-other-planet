...
void draw_giant_planet(SDL_Renderer* renderer, const GameTextures& tex, int w, int h, float time) {
    ...
    float outerR = radius * (1.2f + energy * 0.02f);
    SDL_SetTextureColorMod(tex.playerGlow, 40, 20, 60);
    SDL_SetTextureAlphaMod(tex.playerGlow, (Uint8)(60 + energy * 1));
...
void draw_alien_sky_elements(SDL_Renderer* renderer, const GameTextures& tex, int w, int h, float time) {
    ...
    for (auto& s : stars) {
        ...
        float brightness = twinkle * (0.7f + smoothEnergy * 0.2f);
        if (brightness > 1.0f) brightness = 1.0f;
        ...
        float currentSize = s.baseSize * (1.0f + smoothEnergy * 0.03f);
...
void render_frame(SDL_Renderer* renderer, const GameTextures& tex, bool brushMode, int brushEffectMode, bool playerSunMode, bool playerRainbow, float playerJumpTimer, bool showFPS, float fpsValue) {
    ...
    static std::vector<SDL_Vertex> glowBatch(MAX_RAINBOW_FRAGMENTS * 6);
    SDL_Vertex* gPtr = glowBatch.data();
    int glowCount = 0;
    ...
    for (const auto& rf : rainbowFragments) {
        ...
        if (rf.type == 1 || rf.type == 4) {
            ...
            float life = 1.0f - life_progress;
            float current_h = fmodf(rf.h + rf.t * 0.08f, 1.0f);
            ...
            float alpha_curve = life * life * life;
            a = static_cast<Uint8>(255.0f * alpha_curve * rf.alpha0);
            current_size = rf.size * (0.3f + 0.7f * sqrtf(life));
            float glowSize = current_size * 2.5f;
            float gh = glowSize * 0.5f;
            Uint8 ga = (Uint8)(a * 0.35f);
            SDL_Color gc = { r, g, b, ga };

            gPtr[glowCount++] = { {rf.x - gh, rf.y - gh}, gc, {0, 0} };
            gPtr[glowCount++] = { {rf.x + gh, rf.y - gh}, gc, {1, 0} };
            gPtr[glowCount++] = { {rf.x - gh, rf.y + gh}, gc, {0, 1} };
            gPtr[glowCount++] = { {rf.x + gh, rf.y - gh}, gc, {1, 0} };
            gPtr[glowCount++] = { {rf.x + gh, rf.y + gh}, gc, {1, 1} };
            gPtr[glowCount++] = { {rf.x - gh, rf.y + gh}, gc, {0, 1} };
        }
        float half = current_size * 0.5f;
        SDL_Color col = { r, g, b, a };

        vPtr[vertCount++] = { {rf.x - half, rf.y - half}, col, {0, 0} };
        vPtr[vertCount++] = { {rf.x + half, rf.y - half}, col, {1, 0} };
        vPtr[vertCount++] = { {rf.x - half, rf.y + half}, col, {0, 1} };
        vPtr[vertCount++] = { {rf.x + half, rf.y - half}, col, {1, 0} };
        vPtr[vertCount++] = { {rf.x + half, rf.y + half}, col, {1, 1} };
        vPtr[vertCount++] = { {rf.x - half, rf.y + half}, col, {0, 1} };
    }
    SDL_SetTextureBlendMode(tex.dot, SDL_BLENDMODE_ADD);

    if (glowCount > 0) {
        SDL_RenderGeometry(renderer, tex.dot, glowBatch.data(), glowCount, nullptr, 0);
    }
    if (vertCount > 0) {
        SDL_SetTextureBlendMode(tex.dot, SDL_BLENDMODE_ADD);
        SDL_SetTextureAlphaMod(tex.dot, 80);
        for (int i = 0; i < vertCount; i += 6) {
            float cx = (rainbowBatch[i].position.x + rainbowBatch[i + 4].position.x) * 0.5f;
            float cy = (rainbowBatch[i].position.y + rainbowBatch[i + 4].position.y) * 0.5f;
            float expand = 2.0f;
            for (int j = 0; j < 6; ++j) {
                rainbowBatch[i + j].position.x = cx + (rainbowBatch[i + j].position.x - cx) * expand;
                rainbowBatch[i + j].position.y = cy + (rainbowBatch[i + j].position.y - cy) * expand;
            }
        }
        SDL_RenderGeometry(renderer, tex.dot, rainbowBatch.data(), vertCount, nullptr, 0);
        SDL_SetTextureAlphaMod(tex.dot, 150);
        for (int i = 0; i < vertCount; i += 6) {
            float cx = (rainbowBatch[i].position.x + rainbowBatch[i + 4].position.x) * 0.5f;
            float cy = (rainbowBatch[i].position.y + rainbowBatch[i + 4].position.y) * 0.5f;
            float shrink = 0.7f;
            for (int j = 0; j < 6; ++j) {
                rainbowBatch[i + j].position.x = cx + (rainbowBatch[i + j].position.x - cx) * shrink;
                rainbowBatch[i + j].position.y = cy + (rainbowBatch[i + j].position.y - cy) * shrink;
            }
        }
        SDL_RenderGeometry(renderer, tex.dot, rainbowBatch.data(), vertCount, nullptr, 0);
        SDL_SetTextureAlphaMod(tex.dot, 255);
    ...
    for (const auto& bp : brushParticles) {
        ...
        float alphaVal = (120 + 40 * sinf(bp.t * 1.7f + bp.phase)) * (1.0f + bp.impact * 0.18f);
